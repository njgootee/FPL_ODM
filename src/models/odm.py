...
def odm_model(COMPL_GW, GW_RANGE = 6):
    ...
    for i in range(GW_RANGE+1,COMPL_GW+2):
        ...
        #Support for fringe Cases (extreme DGW/BGW that affect sample size)
        support = 0
        if i in {19,34}:
            support=-1
        if i in {35}:
        ...
        for j in range((i-GW_RANGE)+support,i):
            ...
            gw=pd.read_csv("../../data/interim/gw_clean/gw_clean_"+str(j)+".csv")
    ...
    o_df.to_csv('../../data/processed/offense_scores.csv', index = False)
    d_df.to_csv('../../data/processed/defense_scores.csv', index = False)
